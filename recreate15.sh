...
mkdir -p prisma/migrations/20260719160000_misc_analyzer_field
...
model HuntSession {
  ...
  damageTypes    Json?
  damageSources  Json?
  // From the Miscellaneous Analyzer: Charm/Imbuement/Item Upgrade data. Values are
  // free-form (counts, "18.8k"-style amounts, etc.) so this is stored as raw
  // label/value pairs per section rather than normalized numbers.
  miscData       Json?
  createdAt      DateTime @default(now())
  updatedAt      DateTime @updatedAt
...
cat > "prisma/migrations/20260719160000_misc_analyzer_field/migration.sql" << 'EOF_prisma_migrations_20260719160000_misc_analyzer_field_migration_sql_'
-- AlterTable
ALTER TABLE "HuntSession" ADD COLUMN     "miscData" JSONB;
EOF_prisma_migrations_20260719160000_misc_analyzer_field_migration_sql_
...
git add -A
git commit -m "Add Miscellaneous Analyzer (Charm/Imbuement/Item Upgrade) parsing"
